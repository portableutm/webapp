...
describe('SP1: (Map) Layers', function () {
	...
	it('Visits OperationsList', function () {
		cy.visit('http://localhost:2000/dashboard/operations');
		cy.get('[data-test-id="filter.bytext"]').type('PENDING');
		cy.window().then((win) => {
			expect(win.store.operationStore.counts.matchingTextAndStateCount).to.eq(1);
		});
		cy.get('[data-test-id="opPENDING"]').within(() => {
			cy.get('[data-test-id="showHidePropertiesPENDING"]').click();
			cy.get('[data-test-id="showHidePropertiesPENDING"]').click();
			cy.get('[data-test-id="showinmap"]').click();
		});
		cy.visit('http://localhost:2000/dashboard/operations');
		cy.get('[data-test-id="opPENDING"]').within(() => {
			cy.get('[data-test-id="editinmap"]').click();
			cy.visit('http://localhost:2000/dashboard/operations');
		});
	});

	it('touch everything', function () {
		cy.visit('http://localhost:2000/dashboard/operations');
		cy.contains('filter.accepted').click();
		cy.contains('filter.pending').click();
		cy.contains('filter.activated').click();
		cy.contains('filter.rogue').click();
		cy.contains('filter.closed').click();
		cy.contains('historical_mode_description').click();
	});
});